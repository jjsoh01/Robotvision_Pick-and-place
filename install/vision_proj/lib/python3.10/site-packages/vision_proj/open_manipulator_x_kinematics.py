...
class OpenManipulatorXKinematics:
    def __init__(self, config_file=None):
        ...
        self.L1 = 0.080   # Base to Shoulder (Center of Rotation)
        self.L2 = 0.115  # Shoulder to Elbow (Center of Rotation)
        self.L3 = 0.120   # Elbow to Wrist (Center of Rotation)
        self.L4 = 0.050   # Wrist to End-effector (Tool plate or attachment point, excluding gripper)
    ...
    def forward_kinematics(self, joint_angles):
        ...
        z_eff = L1 + L2 * np.sin(q2) + L3 * np.sin(q2 + q3) + L4 * np.sin(q2 + q3 + q4)
    ...
    def inverse_kinematics(self, target_x, target_y, target_z, initial_joint_angles=None):
        if initial_joint_angles is None:
            initial_joint_angles = np.array([0.0, 0.0, np.pi/2])
        else:
            initial_joint_angles = np.array(initial_joint_angles[:3])  # q1, q2, q3만!
        ...
        def objective_function(joint_angles):
            # q1, q2, q3만 받음, q4는 자동계산
            q1, q2, q3 = joint_angles
            q4 = -(q2 + q3)
            fk_angles = [q1, q2, q3, q4]
            x, y, z = self.forward_kinematics(fk_angles)
