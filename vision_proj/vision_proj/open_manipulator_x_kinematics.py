...
class OpenManipulatorXKinematics:
    ...
    def _load_config(self, filepath):
        try:
            with open(filepath, 'r') as file:
                ...
                print(f"Config 파일 로딩 성공 from {filepath}")
    ...
    def inverse_kinematics(self, target_x, target_y, target_z):
        ...
        # 제2 코사인을 통해 q3 구하기
        cos_q3 = (L2**2 + L3**2 - r_new**2 - z_new**2) / (2*L2*L3)
        cos_q3 = np.clip(cos_q3, -1.0, 0.0)
        q3 = np.arccos(cos_q3)
        q3 = np.pi - q3
        ...
        k1 = L2 + L3 * np.cos(q3) 
        k2 = L3 * np.sin(q3)
        q2 = np.pi/2 - (np.arctan2(z_new, r_new) + np.arctan2(k2, k1))
